<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-3xl md:text-4xl font-medium tracking-tight dark:text-gray-100 text-gray-900">Add Radio Station</h1>
                <p class="mt-1.5 text-gray-500 dark:text-gray-500">Register a known station for cross-device playback</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-2xl">
        <div class="bg-white dark:bg-stone-900 rounded-3xl shadow-lg border border-gray-200/70 dark:border-stone-800/80 p-8 md:p-10">
            <form method="POST" action="{{ route('radio.store') }}">
                @csrf

                @include('radio.partials.form')
                ...
                <div class="mt-8 pt-6 border-t border-gray-100 dark:border-stone-800 flex items-center justify-end gap-3">
                    ...
                    <button type="submit"
                        ...
                        <i class="fa-solid fa-plus mr-2"></i>Add Station
                    </button>
                </div>
            </form>
        </div>
