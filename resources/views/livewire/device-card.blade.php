...
<div wire:poll.1s class="bg-white dark:bg-stone-900 rounded-3xl shadow-lg border border-gray-200/70 dark:border-stone-800/80 overflow-hidden hover:shadow-xl transition-all {{ $isActive ? 'md:col-span-2' : '' }}">
        ...
        @if($isActive)
            ...
            @if($nowPlaying)
                <div class="flex flex-col {{ $isActive ? 'lg:flex-row' : '' }} gap-6">
                    ...
                    {{-- Artwork --}}
                    @php
                        $artUrl = $nowPlaying->track?->images[0]['url'] ?? $nowPlaying->radio?->images[0]['url'] ?? null;
                    @endphp
                    @if($artUrl)
                    ...
                    @else
                        <div class="w-full lg:w-48 aspect-square rounded-2xl bg-gray-100 dark:bg-stone-800 flex items-center justify-center flex-shrink-0">
                            <i class="fa-solid fa-{{ $nowPlaying->radio ? 'tower-broadcast' : 'music' }} text-4xl text-gray-300 dark:text-stone-600"></i>
                        </div>
                    @endif
